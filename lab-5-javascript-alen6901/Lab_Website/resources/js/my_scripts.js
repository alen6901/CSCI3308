...
function viewStudentStats(id, toggle)
 {
   console.log()
  // Element.toggle.Atribute(toggle)
...
function loadStatsPage()
{
  ...
  for (var i = 2; i < table.rows.length; i++) {
    ...
    if(parseInt(row.cells[2].innerHTML) > parseInt(row.cells[3].innerHTML)){
      row.cells[4].innerHTML = "Colorado Boulder"
      wins++
    } 
    else {
...
function loadPlayersPage()
{
  ...
  for(let i = 0; i < players.length; i++)
  {
    ...
    choice.href = '#'
    choice.style = "float : left; clear: left; color : black;"
    dropdown.append(choice)
    choice.onclick = function() {switchPlayers(i)}
  }
...
function switchPlayers(playerNum)
{
  ...
  if(players[playerNum].year == "Senior"){
    document.getElementById("avg_p_yards").innerHTML = players[playerNum].pass_yards/4
    document.getElementById("avg_r_yards").innerHTML = players[playerNum].rushing_yards/4
    document.getElementById("avg_rec_yards").innerHTML = players[playerNum].receiving_yards/4
  }
  else if(players[playerNum].year == "Junior"){
    document.getElementById("avg_p_yards").innerHTML = players[playerNum].pass_yards/3
    document.getElementById("avg_r_yards").innerHTML = players[playerNum].rushing_yards/3
    document.getElementById("avg_rec_yards").innerHTML = players[playerNum].receiving_yards/3
  }
  else if(players[playerNum].year == "Sophomore"){
    document.getElementById("avg_p_yards").innerHTML = players[playerNum].pass_yards/2
    document.getElementById("avg_r_yards").innerHTML = players[playerNum].rushing_yards/2
    document.getElementById("avg_rec_yards").innerHTML = players[playerNum].receiving_yards/2
  }
  else if(players[playerNum].year == "Freshman"){
    document.getElementById("avg_p_yards").innerHTML = players[playerNum].pass_yards
    document.getElementById("avg_r_yards").innerHTML = players[playerNum].rushing_yards
    document.getElementById("avg_rec_yards").innerHTML = players[playerNum].receiving_yards
  }
  return;
}