...
app.get('/team_stats', function(req, res) {
	...
	var query2 = 'select count(home_score) from football_games where home_score > visitor_score;';
...
app.get('/player_info', function(req, res) {
    var query = 'SELECT * FROM football_players;' // Write a SQL query to retrieve the colors from the database
    db.task('get-everything', task => {
        return task.batch([
            task.any(query),
        ]);
...
app.get('/players', function(req, res) {
    var year = req.query.player_year ? req.query.player_year : '';
    var query = year ? `SELECT * FROM football_players WHERE year = '${year}';` :  `SELECT * FROM football_players;`// Write a SQL query to retrieve the colors from the database
    db.task('get-everything', task => {
        return task.batch([
            task.any(query),
        ]);
...
app.get('/players/player_info', function(req, res) {
    ...
    var games = 'SELECT COUNT(*) FROM football_games WHERE ' + player_id + '=ANY(players);';
...
app.post('/players/add', function(req, res) {
    ...
    var player_major = req.body.player_major;
    var player_passing_yards = req.body.player_passing_yards;
    var player_rushing_yards = req.body.player_rushing_yards;
    var player_receiving_yards = req.body.player_receiving_yards;
    var insert_statement = `insert into football_players (name, year, major, passing_yards, rushing_yards, receiving_yards) values ('${player_name}','${player_year}','${player_major}','${player_passing_yards}','${player_rushing_yards}','${player_receiving_yards}');`
...
app.post('/games/add', function(req, res) {
    ...
    var insert_statement = `insert into football_games (visitor_name, home_score, visitor_score, game_date, players) values ('${visitor_name}','${home_score}','${visitor_score}','${game_date}','${players}');`
